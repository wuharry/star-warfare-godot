...
const OUT := "res://test_output/thunder_detail_preview/"
...
var framing_full := AABB()
var framing_head := AABB()
...
func _ready() -> void:
	...
	GameState.save_path = "user://thunder_detail_preview_profile.json"
...
func _run() -> void:
	...
	for detail: String in ["crown", "visor", "respirator", "chest", "shoulder", "forearm", "legs"]:
		_frame_detail(detail)
		await _save("detail_" + detail, "detail", detail, "idle_rifle", 0.0, viewport)
	...
	if output == null:
		...
		output.store_string(JSON.stringify({
			...
			"frames": manifest,
			"failures": failures,
			"notes": "Actual runtime meshes/materials. Studio uses the existing rifle idle pose with weapon/backpack hidden; pose and lighting differ from the concept. Rear is reviewed against original SW2 geometry; the concept only supplies a front three-quarter angle. Baseline and current studio framing use the baseline bounds. Motion/game views use existing game animations and materials.",
		}, "\t"))
		output.close()
	AudioDirector.stop_all_sfx()
	print("THUNDER_DETAIL_CAPTURE_%s stage=%s images=%d save_unchanged=%s" % ["PASS" if failures.is_empty() else "FAIL", stage, manifest.size(), str(final_hash == real_save_hash)])
...
func _capture_gameplay() -> void:
	...
	for level: int in [1, 3]:
		...
		if level == 3:
			...
			var visible_head := world.player.recovered_avatar.find_child("ArmorHead_06", true, false) as MeshInstance3D
			var posed_head := _posed_bounds(visible_head, world.player.recovered_skeleton)
			var center := posed_head.get_center() + Vector3(0, -0.22, 0)
			inspection.global_position = center + world.player.model.global_basis * Vector3(0.28, -0.68, -1.45)
			inspection.look_at(center)
			inspection.current = true
			await _save("game_level_3_low_front", "game", "inspection_low_front", "live_game", 0.0, game_viewport)
		world.queue_free()
		await get_tree().process_frame
...
func _check_parts(player: WarfarePlayer, record: bool) -> void:
	...
	for part: MeshInstance3D in player.recovered_avatar.find_children("Armor*", "MeshInstance3D", true, false):
		...
		if stage == "current" and str(part.get_meta("armor_rework", "")) != "thunder_detail_v4":
			_fail("Current capture requires thunder_detail_v4: " + str(part.name))
		if record:
...
func _frame(view: String, close: bool) -> void:
	var bounds := framing_head if close else framing_full
...
func _load_framing() -> void:
	framing_head = head_bounds
	framing_full = full_bounds
	if stage != "current" or not FileAccess.file_exists(OUT + "baseline/manifest.json"):
		return
	var data: Dictionary = JSON.parse_string(FileAccess.get_file_as_string(OUT + "baseline/manifest.json"))
	for key: String in ["full_bounds", "head_bounds"]:
		var p: Array = data[key].position
		var s: Array = data[key].size
		var bounds := AABB(Vector3(p[0], p[1], p[2]), Vector3(s[0], s[1], s[2]))
		if key == "full_bounds":
			framing_full = bounds
		else:
			framing_head = bounds
...
func _frame_detail(detail: String) -> void:
	var center := framing_head.get_center()
	var span := framing_head.size.x * 1.05
	var view := Vector3(1.1, 0.3, -5)
	match detail:
		"crown":
			center.y = framing_head.position.y + framing_head.size.y * 0.76
			span = framing_head.size.x * 1.05
			view = Vector3(1.7, 2.2, -5)
		"visor":
			center.y = framing_head.position.y + framing_head.size.y * 0.46
			span = framing_head.size.x * 0.92
		"respirator":
			center.y = framing_head.position.y + framing_head.size.y * 0.2
			span = framing_head.size.x * 0.63
			view = Vector3(1.8, -1.4, -5)
		"chest":
			center = framing_full.position + framing_full.size * Vector3(0.5, 0.56, 0.5)
			span = framing_full.size.x * 0.81
		"shoulder":
			center = framing_full.position + framing_full.size * Vector3(0.13, 0.60, 0.5)
			span = framing_full.size.x * 0.52
			view = Vector3(-2, 1, -5)
		"forearm":
			center = framing_full.position + framing_full.size * Vector3(0.30, 0.46, 0.5)
			span = framing_full.size.x * 0.62
			view = Vector3(-2, 0, -5)
		"legs":
			center = framing_full.position + framing_full.size * Vector3(0.5, 0.22, 0.5)
			span = framing_full.size.x * 0.95
	fixture.view_camera.size = span
	fixture.view_camera.position = center + view
	fixture.view_camera.look_at(center)
	fixture.fill.position = fixture.view_camera.position
