// Connect only to the dedicated isolated preview browser on localhost:19447.
import {writeFile, access} from 'node:fs/promises';
import {fileURLToPath} from 'node:url';
const directory=new URL(process.argv.includes('--published')?'../../docs/art/thunder_detail_v4/':'../../test_output/thunder_detail_preview/',import.meta.url);
const tab=await(await fetch('http://localhost:19447/json/new?about:blank',{method:'PUT'})).json();
...
try{
 ...
 assert(await evaluate("$('design-note').textContent.includes('上緣保留斜切與中央下垂')&&$('design-note').textContent.includes('呼吸器厚度向內收')"),'Latest visor/respirator design note missing');
 ...
 const sourceLoaded=await evaluate(`Promise.all(sourceFrames.map(name=>new Promise(resolve=>{const image=new Image();image.onload=()=>resolve(image.naturalWidth>0&&image.naturalHeight>0);image.onerror=()=>resolve(false);image.src='sw2_reference/'+name+'.png'})))`);
 assert(sourceLoaded.every(Boolean),'Missing original SW2 reference render');
 ...
 for(const scope of ['full','helmet','detail']){
  await evaluate(`$('scope').value='${scope}';choices();refresh()`);
  const views=await evaluate("[...$('view').options].map(o=>o.value)");
  for(const view of views){
   await evaluate(`$('view').value='${view}';refresh()`);
   for(const stage of stages)assert(await evaluate(`$('${stage}-frame').querySelector('img').src.endsWith('${stage}/${scope}_${view}.png')`),'Comparison controls failed');
  }
 }
 for(const stage of stages){
  ...
  for(const view of ['three_quarter','side','rear']){
   ...
   assert(await evaluate(`document.querySelectorAll('#motion-gallery img').length===4&&[...document.querySelectorAll('#motion-gallery img')].slice(1).every(i=>i.src.endsWith('_${view}.png'))`),'Motion controls failed');
  }
  assert(await evaluate("document.querySelectorAll('#game-gallery img').length===3"),'Gameplay gallery incomplete');
 }
 await evaluate("$('reset').click();$('motion-view').value='three_quarter';refresh();$('baseline-frame').querySelector('img').click()");
 assert(await evaluate("$('zoom').open&&$('zoom-image').src===$('baseline-frame').querySelector('img').src"),'Zoom failed');
 await evaluate("$('zoom-close').click()");
 assert(await evaluate("!$('zoom').open&&$('view').value==='three_quarter'&&$('scope').value==='helmet'"),'Close/reset failed');
 ...
 const report={status:'PASS',stages,images:loaded.length,reference:true,sw2_source_images:sourceLoaded.length,camera_controls:true,motion_controls:true,zoom:true,local_links:links.length,desktop:'1440x1150',mobile:'390x844',horizontal_overflow:false,browser_errors:errors};
 await writeFile(new URL('browser_report.json',directory),JSON.stringify(report,null,2));
 console.log('THUNDER_DETAIL_PREVIEW_BROWSER_PASS '+JSON.stringify(report));
