...
const OUT := "res://test_output/thunder_helmet_comparison/"
...
func _ready() -> void:
	...
	GameState.save_path = "user://thunder_helmet_comparison_profile.json"
	scene_hash = FileAccess.get_sha256(preload("res://scripts/game/armor_visuals.gd").reworked_scene_path(6))
...
func _run() -> void:
	...
	if output == null:
		...
		output.store_string(JSON.stringify({
			...
			"frames": manifest,
			"failures": failures,
			"notes": "Actual runtime meshes/materials. Studio uses the existing rifle idle pose with weapon/backpack hidden; pose and lighting differ from the concept. A retains the SW2 back; B retains the prototype back. Both use the same stored V3 framing bounds; the concept only supplies a front three-quarter angle. Motion/game views use existing game animations and materials.",
		}, "\t"))
		output.close()
	AudioDirector.stop_all_sfx()
	print("THUNDER_HELMET_CAPTURE_%s stage=%s images=%d save_unchanged=%s" % ["PASS" if failures.is_empty() else "FAIL", stage, manifest.size(), str(final_hash == real_save_hash)])
...
func _load_framing() -> void:
	...
	if not FileAccess.file_exists("res://tools/thunder_helmet_comparison/framing.json"):
		return
	var data: Dictionary = JSON.parse_string(FileAccess.get_file_as_string("res://tools/thunder_helmet_comparison/framing.json"))
