...
- 複雜名詞第一次出現，三件事都要給，且用條列或表格呈現、不寫成一段話：① 白話定義 ② 具體例子（用當前專案的真實程式或數字） ③ 什麼情境會遇到。
- **白話但不失專業：抽象詞下一句就要落地。** 用了術語或抽象詞，緊接著給一個看得到的東西——真實變數名、真實數字、真實行號，或一句白話重述。目標是把機制講到看得見，不是把機制講不見。
