...
## Assault Armor style prototype (2026-09-10 v3)

Set 21 features fully beveled, rounded limbs and dedicated dark metal textures
with bright orange accents, eliminating the low-poly boxy limb appearance and
color mismatch. Hands, feet and torso utilize dedicated textures (`hand_assault.png`,
`foot_assault.png`, `body_assault.png`) that strictly match `equip.png` and `helmet.png`.
In Blender, forearm bracers, fist edges, and combat boots undergo automated beveling
and subtle curvature sculpts with `ArmorForm` vertex colors.
All meshes retain the shared player skin and animation sockets. `fit_report.json`
describes the baseline retarget; `assault_refinement_report.json` records this pass.

Reapply the prototype pipeline:

```powershell
python tools/callofmini_enhancer/generate_assault_limb_textures.py
.\.tools\godot-4.7.2\Godot_v4.7.2-stable_win64_console.exe --headless --path . --script res://tools/callofmini_enhancer/refine_assault.gd -- --export --force
